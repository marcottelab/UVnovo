...
% 
% Andrew Horton modifications:
%	Return return >1 rgb value when multiple colorname arguments.
% 	Also, RGB(N) returns n random colors (sorted, white not included).
%	Default argument 'chart' if no args provided.
%	There are a few new colors with poorly-chosen names. ex. 'pptishdkgreen'
%	RGB = RGB('GETCOLORS') returns a struct of all colors defined here.

function rgb = rgb(varargin)
  if isempty(varargin), varargin = {'chart'}; end
  s = varargin;
  if numel(s)==1 && iscell(s{1}), s = s{1}; end
  persistent num name
  ...
  if isnumeric(s{1})
    ...
    s = regexprep(s,' ','');
	if strcmpi(s{1},'getColors')
		rgb = cell2struct( mat2cell(num, ones(numel(name),1), 3), name, 1);
		return
	end
    ncolors = numel(s);
    if ncolors==1 && strcmpi(s,'chart')
      showcolors()
    else
      rgb = zeros(ncolors,3);
      for i = 1:ncolors
        k = find(strcmpi(s{i}, name));
        if isempty(k)
          error(['Unknown color: ' s{i}]);
        else
          rgb(i,:) = num(k(1), :);
        end
      end
